/* =========================================================
   ...
   app.js v3.1.0 (شحن المحفظة - ليبيانا / مدار)
   ========================================================= */
...
(() => {
    ...
    /* حالة شحن المحفظة */
    let topupCardType = "libyana";
    let topupCardAmount = 5;
    ...
    const translations = {
        ar: {
            ...
            /* شحن المحفظة */
            topupPending: "قيد المراجعة",
            topupApproved: "تم الشحن",
            topupRejected: "مرفوض",
            topupInvalidCard: "الكرت غير صالح، يرجى التحقق من صحته.",
            topupSent: "تم إرسال الكرت للمراجعة بنجاح.",
            topupError: "تعذر إرسال الكرت.",
            topupNoHistory: "لا توجد طلبات شحن بعد.",
            topupEnterCode: "يرجى إدخال رمز الكرت أولاً."
        }
    };
    ...
    async function submitTopup() {
        ...
        try {
            ...
            /*
             * تفريغ الحقل وإعادة تحميل السجل.
             */
            if (codeInput) {
                codeInput.value = "";
            }
            ...
            if (button) {
                ...
                button.innerHTML = `
                    ${icons.send}
                    <span>إرسال الكرت للمراجعة</span>
                `;
            }
        }
    }
    ...
    function getTopupStatusLabel(status) {
        ...
        if (value === "approved") {
            return t.topupApproved;
        }

        if (value === "rejected") {
            return t.topupRejected;
        }

        return t.topupPending;
    }
    ...
    function createTopupRow(topup) {
        ...
        /*
         * زر التواصل مع الدعم يظهر
         * بجانب الطلبات المرفوضة.
         */
        const contactButton = isRejected
            ? `
                <button
                    type="button"
                    data-topup-contact="1"
                    style="
                        margin-top:8px;
                        height:32px;
                        padding:0 10px;
                        border-radius:9px;
                        color:var(--blue);
                        border:1px solid rgba(57,152,255,.25);
                        background:rgba(57,152,255,.07);
                        font-size:9px;
                        font-weight:900;
                        display:inline-flex;
                        align-items:center;
        ...
        return `
            <article class="order-card">
                ...
                <div class="order-top">
                    ...
                    <div
                        class="order-status ${getTopupStatusClass(
                            status
                        )}"
                    >
                        ${escapeHtml(
    ...
    function attachTopupButtons() {

        $all("[data-topup-contact]")
            .forEach(button => {
