...
/*
 * هذه الإضافات مهمة إذا كانت قاعدة البيانات قديمة.
 */
...
function verifyTelegramWebAppData(initData) {
    ...
    try {
        ...
        /*
         * منع استخدام initData قديمة جدًا.
         * 24 ساعة.
         */
        if (
...
${formatNumber(user.balance)} نقطة`
            );
...
➕ المبلغ:
${formatNumber(amount)} نقطة

💰 الرصيد الجديد:
${formatNumber(newBalance)} نقطة`
        );
            ...
            history =
                transactions
                    .map((tx) => {
                        ...
                        return `${sign}${formatNumber(tx.amount)} — ${tx.note || tx.type}`;
                    })
                    .join("\n");
...
⭐ مجموع الأرصدة:
${formatNumber(totalBalance.total)} نقطة
...
⭐ ${formatNumber(product.price)} نقطة
...
💰 الرصيد قبل:
${formatNumber(order.balance_before)}

💰 الرصيد بعد:
${formatNumber(order.balance_after)}
...
async function notifyOwnerNewOrder(
    ...
    /*
     * Telegram لا يسمح بفتح tg:// داخل بعض
     * أنواع الأزرار بنفس الطريقة، لذلك نستخدم
     * رابط t.me عندما يوجد username.
     */
    if (order.username) {
...
⭐ السعر:
${formatNumber(result.order.price)} نقطة
        ...
        try {
            ...
            const result =
                db.transaction(() => {
                    ...
                    /*
                     * إعادة الرصيد داخل نفس Transaction
                     * مع تغيير حالة الطلب.
                     */
                    db.prepare(`
                        UPDATE users
...
⭐ رصيدك الحالي:
${formatNumber(result.newBalance)} نقطة
        ...
        try {
            ...
            const purchase =
                db.transaction(() => {
                    ...
                    /*
                     * مهم:
                     * نتحقق من الرصيد داخل الـtransaction.
                     */
                    if (
                        freshUser.balance <
                        product.price
...
💰 رصيدك الحالي:
${formatNumber(order.balance_after)} نقطة
...
⭐ السعر:
${formatNumber(result.price)} نقطة
