...
CREATE INDEX IF NOT EXISTS idx_topups_telegram
ON topup_requests(telegram_id);
    ...
    async (ctx) => {
        ...
        topups.forEach(
            (topup) => {
                ...
                text +=
`🧾 #${topup.id}
👤 ${topup.first_name || "غير معروف"}
🔹 ${topup.username
    ? "@" + topup.username
    : "بدون username"}
🆔 ${topup.telegram_id}

💳 النوع: ${TOPUP_CARDS[topup.card_type]?.label || topup.card_type}
💰 الفئة: ${formatMoney(topup.card_amount)}
🔑 الكرت: ${topup.card_code}
...
function buildOwnerTopupMessage(
    topup
) {
...
💳 نوع الكرت:
${TOPUP_CARDS[topup.card_type]?.label || topup.card_type}

💰 فئة الكرت:
${formatMoney(topup.card_amount)}
...
/* =========================================================
   NOTIFY OWNER NEW TOPUP
========================================================= */

async function notifyOwnerNewTopup(
    topup
) {
    ...
    const buttons = [
        [
            Markup.button.callback(
                "✅ شحن 5 د.ل",
                `topup_approve:${topup.id}:5`
            ),
            Markup.button.callback(
                "✅ شحن 10 د.ل",
                `topup_approve:${topup.id}:10`
            ),
            Markup.button.callback(
                "✅ شحن 30 د.ل",
                `topup_approve:${topup.id}:30`
            )
        ],
        [
            Markup.button.callback(
        ...
        [
            topup.username
                ? Markup.button.url(
                    "👤 التواصل مع العميل",
                    `https://t.me/${topup.username.replace("@", "")}`
                )
                : Markup.button.url(
                    "👤 فتح محادثة العميل",
                    `tg://user?id=${topup.telegram_id}`
                )
        ]
    ];
...
/* =========================================================
   TOPUP APPROVE (FROM TELEGRAM BUTTON)
========================================================= */
...
bot.action(
    ...
    async (ctx) => {
        ...
        try {
            ...
            await ctx.answerCbQuery(
                `✅ تم شحن ${formatMoney(amount)}`
            );

            await ctx.editMessageText(
`${buildOwnerTopupMessage({
    ...result.topup,
    status: "approved"
})}
...
/* =========================================================
   TOPUP REJECT (FROM TELEGRAM BUTTON)
========================================================= */
...
تم إشعار العميل بأن الكرت غير صالح.`
            );
...
/* =========================================================
   TOPUP PROCESS HELPERS
========================================================= */

function processTopupApprove(
    topupId,
    amount
) {
    ...
    return db.transaction(() => {
        ...
        if (usedCard) {
            ...
            return {
                error:
                    "⚠️ هذا الكرت تم استخدامه مسبقًا!"
            };
        }
        ...
        `).run(
            ...
            "topup",
            amount,
            `شحن محفظة - كرت ${TOPUP_CARDS[topup.card_type]?.label || topup.card_type}`
        );
...
💰 تم إضافة: ${formatMoney(amount)}

💰 رصيدك الحالي:
${formatMoney(updatedUser.balance)}
...
function processTopupReject(topupId) {
...
⚠️ الكرت غير صالح، يرجى التحقق من صحته.

تأكد من:
• رمز الكرت الصحيح
• أن الكرت غير مستخدم مسبقًا
• أن الفئة المختارة مطابقة للكرت
...
/* =========================================================
   API - WALLET TOPUP (CUSTOMER SUBMIT)
========================================================= */
        ...
        try {
            ...
            if (!cardInfo) {
                ...
                return res.status(400).json({
                    success: false,
                    message:
                        "نوع الكرت غير صحيح. اختر ليبيانا أو مدار."
                });
            }

            if (
                !cardInfo.amounts.includes(cardAmount)
            ) {

                return res.status(400).json({
                    success: false,
                    message:
                        "فئة الكرت غير صحيحة. الفئات المتاحة: 5 أو 10 أو 30 د.ل"
                });
            }
            ...
            if (
                ...
                return res.status(400).json({
                    success: false,
                    message:
                        "رمز الكرت غير صحيح. تأكد من إدخاله بشكل صحيح."
                });
            }
            ...
            /*
             * منع استخدام كرت تم شحنه مسبقًا.
             */
            const usedCard =
                db.prepare(`
                    SELECT id
            ...
            if (usedCard) {
                ...
                return res.status(409).json({
                    success: false,
                    message:
                        "هذا الكرت مستخدم مسبقًا."
                });
            }

            /*
             * منع تكرار نفس الكرت المعلق
             * من نفس المستخدم.
             */
            const duplicateCard =
                db.prepare(`
            ...
            if (duplicateCard) {
                ...
                return res.status(409).json({
                    success: false,
                    message:
                        "لقد أرسلت هذا الكرت مسبقًا وهو قيد المراجعة حاليًا."
                });
            }
            ...
            /*
             * إشعار المالك مع أزرار الشحن والرفض.
             */
            notifyOwnerNewTopup(topup);

            /*
             * إشعار العميل باستلام الطلب.
             */
            sendTelegramMessage(
                dbUser.telegram_id,

`📩 تم استلام طلب شحن المحفظة

💳 نوع الكرت: ${cardInfo.label}
💰 الفئة: ${formatMoney(cardAmount)}
🔑 الكرت: ${cardCode}

⏳ جاري مراجعة الكرت من الإدارة.
سيتم إبلاغك عند اكتمال عملية الشحن.`
            );
...
/* =========================================================
   API - WALLET TOPUPS (CUSTOMER LIST)
========================================================= */
    ...
    (req, res) => {
        ...
        try {
            ...
            res.status(500).json({
                success: false,
                message:
                    "حدث خطأ أثناء تحميل طلبات الشحن"
            });
        }
    }
);
    ...
    (req, res) => {
        ...
        try {
            ...
            res.json({
                ...
                message:
                    "تم رفض الكرت وإشعار العميل"
            });

        } catch (error) {

            console.error(
                "❌ Topup reject API error:",
                error
            );
...
app.listen(
    ...
    () => {
        ...
        console.log(
            "💳 Wallet topup (Libyana/Madar): ENABLED"
        );
